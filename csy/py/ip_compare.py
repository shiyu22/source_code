...
NQ_FOLDER_NAME = '/home/pensees/feature_compare/capture_csv/test10'
NQ = 0
TOPK = 0
ALL = False
# CSV = False
...
nq_scope = [1]
topk_scope = [1]
# nq_scope = [1,50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000]
...
def load_nq_vec(nq):
    ...
    for filename in filenames:
        vec_list = load_vec_list(FOLDER_NAME+'/'+filename)
        length += len(vec_list)
        if(length>nq):
            flag =1
            num = length%nq
            if (num!=0 and flag==0):
                vec_list = load_vec_list(FOLDER_NAME+'/'+filename,num)
            else:
                vec_list = load_vec_list(FOLDER_NAME+'/'+filename,nq)
        vectors += vec_list
        if len(vectors)==nq :
            return vectors

def load_nq_vec_ps(nq):

    filenames = os.listdir(NQ_FOLDER_NAME)  # 得到文件夹下的所有文件名称
    filenames.sort()
    nq_0 = nq
    vec_list = []
    print(len(filenames))
...
def get_ground_truth(nq, topk ,idx,vct_nq):
    filenames = os.listdir(FOLDER_NAME)  # 得到文件夹下的所有文件名称
    # filenames.sort(key=lambda x: int(x.split('.')[0][-5:]))
    filenames.sort()
    no_dist = {}
    ...
    fname_dis = 'l2_dis_results'+str(idx)+'.txt'

    for n in no_dist:
        print('n[0]:',n[0],'   n[1]',n[1])
        num = n[0]
        topk_list.append(topk_vec[num])
        print(topk_vec[num])
        
        # print("debug")
        # np.save(fname,vet_list[num])
        with open(fname_dis,'a') as f:
            f.write(str(n[1]) + '\n')
        # num = "%03d%06d\n" % (n[0]//nb,n[0]%nb)
        # re.append(num)
    np.save(fname, topk_list)
    # f.close()
    # save_gt_file(re,idx)

# generate the ground truth file
def get_ground_truth_ip(nq, topk ,idx,vct_nq):
    filenames = os.listdir(FOLDER_NAME)  # 得到文件夹下的所有文件名称
    # filenames.sort(key=lambda x: int(x.split('.')[0][-5:]))
    filenames.sort()
    no_dist = {}
    ...
    for filename in filenames:
        ...
        for j in range(len(vet_list)):
            num_j = j
            dist = calInnerDistance(vct_nq,vet_list[j])
    ...
    print(no_dist)
    fname = 'ip_results'+str(idx)+'.npy'
    fname_dis = 'ip_dis_results'+str(idx)+'.txt'
...
def ground_truth_process(nq,topk):
    ...
    for loop in range(loops):
        ...
        for i in range(process_num):
            ...
            process = Process(target=get_ground_truth_ip, args=(nq, topk, base+i ,vectors[base+i]))
...
# save date(ids,maps)
def save_id_to_file(results,table_name,gt = False):
...
def result_output_all(nq,topk,com_vec,rand,num,accuracy,time_cost,results,topk_ground_truth,accuracy_all):
    # random = nowTime=datetime.datetime.now().strftime("%m%d%H%M")
...
def result_output(nq,topk,com_vec,rand,num,accuracy,time_cost,results,accuracy_all):
    # random = nowTime=datetime.datetime.now().strftime("%m%d%H%M")
    filename = str(nq)+"_"+str(topk) + '_result.csv'
    # filename = 'result.csv'
    count=0
    with open(filename,'w') as f:
...
def search_vec_list(table_name,nq=0,topk=0,all_out=ALL):
    ...
    #for i in range(FILE_NUM):
        #file_ids.append(FILE_START + i*2)
    if nq!=0 and topk!=0:
        if NQ==0 and TOPK==0:
        ...
        time_start = time.time()
        save_id_to_file(results, table_name, gt = True)
        time_end = time.time()
        time_cost=time_end - time_start
        ...
        filename = str(random1) + '_results.csv'
        ...
        for nq in nq_scope:
            ...
            for k in topk_scope:
                ...
                file.write(line)
                print(nq, k, time_end - time_start)
            file.write('\n')
        file.close()
...
def search_binary_vec_list(table_name,nq,k):
    ...
    status, results = MILVUS.search_vectors(table_name=table_name, query_records=query_list, top_k=k)
    time_end = time.time()
    save_id_to_file(results, table_name=table_name)
...
def main():
    try:
        opts, args = getopt.getopt(
            sys.argv[1:],
            "hlsgtan:m:q:k:bdp",
            ["help", "list", "search","generate","transform","delete","nb=","ivf=","flat=","table=","num=","nq=","topk=","index=","rows","show","compare","add"],
        )
