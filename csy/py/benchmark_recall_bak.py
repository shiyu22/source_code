...
nq_scope = [1,10,30,50,100,200,500]
topk_scope = [1,1,10,50,100]
...
def load_nq_vec(nq):
    ...
    length = 0
    flag = 0
    filenames = os.listdir(FOLDER_NAME)  # get the whole file names
...
def search_binary_vec_list(table_name,nq,k):
    ...
    time_end = time.time()
    time_cost = time_end-time_start
    print(nq,k,"time_search = ",time_cost)
    time_start = time.time()
    save_id_to_file(results, table_name=table_name)
    time_end = time.time()
    print('compare_time = ',time_end - time_start)
...
def add_somefiles_vec_to_milvus(nfiles = 0, table_name= ''):
    ...
    for filename in filenames:
        ...
        time_l_2 = time.time()
        print("load_cost_time=",time_l_2-time_l_1)
...
def main():
    ...
    for opt_name, opt_value in opts:
        if opt_name in ("-h", "--help"):
            ...
            time_s_2 = time.time()
            print("add_time_cost:",time_s_2 - time_s_1)
